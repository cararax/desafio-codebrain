...
public interface SaleRepository extends JpaRepository<Sale, Long> {
    ...
    @Query(value = "select se.id as \"SellerId\", se.name as \"SellerName\", AVG(sa.total_amount) as \"SalesTicket\" from seller se, sale sa where se.id = sa.seller_id and se.id = :sellerId and sa.sale_date  BETWEEN :startDate AND :endDate \n  group by se.id", nativeQuery = true)
